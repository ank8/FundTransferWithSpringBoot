...
class BankCatalogControllerTests {
	...
	BankCatalog mockBankCtlg = new BankCatalog(1l, "ICICI", 125l, "A");
	...
	void addBank() throws Exception {
		...
		Mockito.when(bankCatalogService.addBanks(Mockito.any(BankCatalogModel.class))).thenReturn(mockBankCtlg);
	...
	void addBankNegative() throws Exception {
		...
		Mockito.when(bankCatalogService.addBanks(Mockito.any(BankCatalogModel.class))).thenThrow(exception);
		RequestBuilder requestBuilder = MockMvcRequestBuilders
				.post(CommonConstants.BASE_PATH + CommonConstants.BANK_PATH).accept(MediaType.APPLICATION_JSON)
				.content(mockContent).contentType(MediaType.APPLICATION_JSON);
		MvcResult result = mockMvc.perform(requestBuilder).andReturn();
	...
	void getAllBank() throws Exception {
		List<BankCatalog> list = new ArrayList<>();
	...
	void updateBankStatus() throws Exception {
		Mockito.when(bankCatalogService.updateBanks(Mockito.any(BankCatalogModel.class))).thenReturn(mockBankCtlg);
		RequestBuilder requestBuilder = MockMvcRequestBuilders
